...
- **Project map / topology.** `assets/` holds the normative help texts (`help*.txt`, single source of truth) embedded via `assets/assets.go`. `cmd/gotto-hando/` is the CLI entry point: argv scanning, `--help*`/`--version`/`--expect-version`, destination and line collection, dispatch. `internal/output/` owns the `E_*` error codes, the 0-5 exit-code mapping, and the plain/JSONL result/done/abort writers. `internal/syntax` is the pure text→IR parser (plus a post-parse `[f]` inline pass); `internal/ir` holds the IR types, static validation/LIMITS, and the `--ir`/`--check` serializer; `internal/backend` defines the thin `Backend` interface (with a test-only `dryrun` impl never bound to the CLI); `internal/engine` executes IR against a backend (composition, held-key tracking, delay precedence, fail-fast/`-k`, and — from the darwin backend on — a pre-op `Preflight` whose `backend.PreflightError` maps to the `abort` object). `internal/backend/darwin` and `internal/backend/windows` are the two real backends (purego on macOS, `golang.org/x/sys/windows` LazyDLL on Windows; both `CGO_ENABLED=0`): both run the keyboard/mouse/scroll/clipboard/text/query command set behind the five-check preflight gate, and darwin additionally runs its Phase 2 — window control (`win`/`qwin`), screen capture (`cap`, whose raw pixels the engine encodes to PNG), and the Screen-Recording-gated capture check plus `--request-perms` — and its Phase 3, `exec` (via `os/exec`) and `open` (with `wait=` window polling on the shared 100 ms loop) — so `local` executes the full command set for real on macOS (Windows is still at the input/query set; its own window/capture/exec phase is a later ticket). `internal/bridge` is the GOOS-agnostic session-bridge core (runs a forwarded IR through the engine against an injected backend, streaming the same JSONL back over one connection, one run at a time); `gotto-hando --bridge` runs it as a resident listener in the GUI session (Windows named pipe with a current-user ACL so far - macOS unix socket is a later phase), and `local` in a remote (ssh) Windows session forwards its run to that bridge (`session=bridge`) instead of injecting in-process, aborting E_SESSION when no bridge is reachable. The full remote (ssh) `<dest>` transport is still unbuilt (a later ticket), and `local` on any other OS plus remote destinations still exit 2. `ai-docs/spec/` holds pointer specs — one `{#stem}` anchor per help-text section — and drift tests across the packages fail whenever code and help text diverge.
- **Canonical flows.** `gotto-hando <dest> [options] [line ...]` parses argv, collects lines (argv or `-f`/stdin), parses each line into IR, and statically validates it. `--check` and `--ir` stop there (exit 0); `local` on macOS or Windows then runs each op against the platform backend (Preflight, then execution, streaming results), while `local` on any other OS and remote destinations still exit 2 pending their backends. Output streams as plain result lines or `--jsonl` objects; `--help*` short-circuits to the embedded asset. See `assets/help.txt` for the full contract.
