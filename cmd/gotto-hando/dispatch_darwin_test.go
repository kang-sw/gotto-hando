...
// ssh-side process. These ops remain exempt from the bridge backend's own
// session preflight; that exemption must not bypass the bridge transport.
func TestRemoteQueryOnlyRunUsesBridgeContext(t *testing.T) {
	remoteEnv(t)
	be := &dryrun.Backend{}
	withBridgeSession(t, &bridge.Session{Backend: be})
